...
img('output/vis_04_regime_bar.png', w=6.2,
    cap='[그림 3] 시기별·지역별 누적 가격 변화율 — 같은 이벤트에도 지역마다 반응이 다름')
...
img('output/fig_02_acf_pacf.png', w=6.2,
    cap='[그림 4] ACF / PACF (1차 차분) — 6개 구, lag=30')
...
h2('2.2 주기성 및 계절성 분석')
img('output/fig_03_fft.png', w=6.2, cap='[그림 5] FFT 주기성 분석')
bul('FFT 분석 결과: 주요 주기 42~63주 — 연간 계절성(52주)보다 장기 사이클에 해당')
img('output/fig_04_decomposition.png', w=6.2,
    cap='[그림 6] 시계열 분해 (원시 / 추세 / 계절 / 잔차) — 6개 구')
bul('계절 분해(seasonal_decompose) 결과: Seasonal 성분이 Trend 대비 극소 — 분해 그림 3열에서 직접 확인')
img('output/vis_03_seasonality.png', w=6.2,
    cap='[그림 7] 연도×월별 주간 변화율 히트맵 — 계절 반복 패턴 부재 확인')
...
img('output/fig_05_correlation.png', w=6.2,
    cap='[그림 8] 구별 가격지수 상관관계 — 수준 상관(좌) vs 차분 상관(우)')
bul('상관관계: 강남 3구 간 > 외곽 3구 간 > 강남-외곽 간 순')
bul('수준 상관: 노원-강남 –0.286(음의 상관) / 차분 상관: +0.420(양의 상관) — 추세 방향은 반대, 변화율 방향은 동조')
img('output/vis_05_lead_lag.png', w=6.0, cap='[그림 9] 강남·서초 vs 노원 선행 효과')
...
img('output/vis_07_irf.png', w=6.2,
    cap='[그림 10] VAR 충격반응함수(IRF) — 강남구 충격에 대한 6개 구 반응')
...
h2('4.1 손실 곡선')
img('output/fig_06_loss_curves.png', w=6.2, cap='[그림 11] 모델별 학습/검증 손실 곡선 (log scale)')
...
h2('4.2 모델 종합 성능')
img('output/vis_06_model_compare.png', w=5.8, cap='[그림 12] 모델별 평균 RMSE·DA 비교')
...
h2('4.5 최종 예측 결과')
bul('모델별 예측 vs 실제값 비교 (테스트 구간)')
img('output/fig_07_final_SimpleRNN.png', w=5.8,
    cap=f'[그림 13] SimpleRNN 예측 vs 실제 — RMSE {avgs["SimpleRNN"][0]:.3f}')
img('output/fig_07_final_LSTM.png', w=5.8,
    cap=f'[그림 14] LSTM 예측 vs 실제 — RMSE {avgs["LSTM"][0]:.3f}')
img(f'output/fig_07_final_{best_nn}.png', w=6.2,
    cap=f'[그림 15] {best_nn} 테스트 구간 예측 vs 실제 / 향후 3주 예측(★) — RMSE {best_rmse:.3f}')
