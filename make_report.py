...
img('output/fig_02_acf_pacf.png', w=6.2,
    cap='[그림 1] ACF / PACF (1차 차분) — 6개 구, lag=30')
...
h2('2.2 주기성 및 계절성 분석')
img('output/fig_03_fft.png', w=6.2, cap='[그림 2] FFT 주기성 분석')
...
img('output/vis_05_lead_lag.png', w=6.0, cap='[그림 3] 강남·서초 vs 노원 선행 효과')
...
img('output/vis_07_irf.png', w=6.2,
    cap='[그림 4] VAR 충격반응함수(IRF) — 강남구 충격에 대한 6개 구 반응')
...
h2('4.1 손실 곡선')
img('output/fig_06_loss_curves.png', w=6.2, cap='[그림 5] 모델별 학습/검증 손실 곡선 (log scale)')
...
h2('4.2 모델 종합 성능')
img('output/vis_06_model_compare.png', w=5.8, cap='[그림 6] 모델별 평균 RMSE·DA 비교')
...
img(f'output/fig_07_final_{best_nn}.png', w=6.2,
    cap=f'[그림 7] {best_nn} 테스트 구간 예측 vs 실제 / 향후 3주 예측(★)')
