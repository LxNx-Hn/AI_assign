...
img('output/fig_02_acf_pacf.png', w=6.2,
    cap='[그림 2] ACF / PACF (1차 차분) — 6개 구, lag=30')
...
h2('2.2 주기성 및 계절성 분석')
img('output/fig_03_fft.png', w=6.2, cap='[그림 3] FFT 주기성 분석')
...
img('output/vis_05_lead_lag.png', w=6.0, cap='[그림 4] 강남·서초 vs 노원 선행 효과')
...
img('output/vis_07_irf.png', w=6.2,
    cap='[그림 5] VAR 충격반응함수(IRF) — 강남구 충격에 대한 6개 구 반응')
...
bul('평가 방식 구분:')
bul('ARIMA: 매 시점 실제값 갱신 후 1-step 예측 → 롤링 단기예측 (직전값 닻 효과)', sub=True)
bul('딥러닝: 고정 입력 시퀀스로 t+1·t+2·t+3 동시 예측 → 3-step 직접 다중 출력', sub=True)
bul('두 방식은 구조적으로 다름 — RMSE 직접 비교 시 이 차이를 감안해야 함', sub=True)
bul('딥러닝 모델 선택 기준: 3-step RMSE 상위 후보 중 DA(방향정확도) 최대 모델 — 방향 신뢰성 우선')
...
h2('4.1 손실 곡선')
img('output/fig_06_loss_curves.png', w=6.2, cap='[그림 6] 모델별 학습/검증 손실 곡선 (log scale)')
...
h2('4.2 모델 종합 성능')
img('output/vis_06_model_compare.png', w=5.8, cap='[그림 7] 모델별 평균 RMSE·DA 비교')
...
doc.add_paragraph().paragraph_format.space_after = Pt(4)
bul(f'ARIMA(walk-forward): RMSE {arima_avg_rmse:.3f}, DA {arima_avg_da:.1f}% — 전체 RMSE 1위')
bul(f'최우수 NN {best_nn}: RMSE {best_rmse:.3f}, DA {best_da:.1f}%, R² {best_r2:.3f}')
bul('완만한 회복기(2025~2026) 데이터에서 ARIMA 우위는 random-walk에 가까운 시계열 특성 반영')
bul('ARIMA는 매 스텝 직전 실제값을 닻으로 예측하나, 레짐 변화 시 즉시 성능 저하')
callout(f'[평가 방법 차이 주의]  ARIMA: 1-step 롤링 예측(매 시점 실제값 갱신) vs 딥러닝: 3-step 직접 다중 출력 — '
        f'구조가 달라 RMSE 단순 수치 비교는 불완전함  |  딥러닝 최우수 모델은 동일 조건(3-step) 내 비교로 선정')
...
img(f'output/fig_07_final_{best_nn}.png', w=6.2,
    cap=f'[그림 8] {best_nn} 테스트 구간 예측 vs 실제 / 향후 3주 예측(★)')
...
for ri, dt in enumerate(dates):
    ...
    is_t3 = (dt == '2026-05-25')
    bg    = 'FFF9C4' if is_t3 else ('F5F5F5' if ri % 2 else 'FFFFFF')
...
# ARIMA vs NN 비교 표
bul('ARIMA vs ' + best_nn + ' 2026-05-25 비교:')
...
for ri, d in enumerate(DISTRICTS):
    ...
    ar   = fc_ar.get(d, 0); nn = fc_nn.get(d, 0); diff = nn - ar
    bg   = 'F0F4FF' if ri % 2 == 0 else 'FFFFFF'
    cell(row.cells[0], d,              bg=bg)
    cell(row.cells[1], f'{ar:.2f}',    size=10, bg='FFF9C4')
    cell(row.cells[2], f'{nn:.2f}',    bold=True, size=10, bg='C8E6C9')
...
cell(sr.cells[1], f'{a_avg:.2f}', bold=True, size=10, bg='FFF9C4')
cell(sr.cells[2], f'{n_avg:.2f}', bold=True, size=10, bg='C8E6C9')
cell(sr.cells[3], f'{n_avg - a_avg:+.2f}', bold=True, size=10, bg='E8EAF6')
doc.add_paragraph().paragraph_format.space_after = Pt(4)
bul(f'두 모델 평균 차이: {n_avg - a_avg:+.2f} — 방향 일치 여부가 예측 신뢰성의 핵심 기준')
...
h2('5.2 모델 성능 결론')
bul(f'ARIMA(1-step walk-forward): RMSE {arima_avg_rmse:.3f}, DA {arima_avg_da:.1f}% — 수치상 1위')
bul(f'최우수 NN {best_nn}(3-step direct): RMSE {best_rmse:.3f}, DA {best_da:.1f}%, R² {best_r2:.3f}')
bul(f'{best_nn}를 최종 예측 모델로 선택한 이유:')
bul('ARIMA는 1-step 롤링(직전 실제값 참조)으로 단기 추종에 유리 — 3주 앞 예측에는 구조적 한계', sub=True)
bul(f'딥러닝 3종 동일 조건(3-step direct) 비교 시 {best_nn}가 RMSE·DA 복합 최우수', sub=True)
bul('6개 구 지역 간 관계, 비선형 패턴 동시 학습 — 단변량 ARIMA 대비 구조적 강점', sub=True)
bul('레짐 변화(급격한 정책·금리 변동) 시 ARIMA 즉시 성능 저하, NN 상대적 강건성', sub=True)
bul('ARIMA 우위 배경: 완만한 회복기(2025~2026)에서 직전값 닻 전략이 random-walk에 최적화된 결과')
bul('방법론: 차분 타겟 전략으로 훈련·테스트 분포 불일치 해소, MinMaxScaler는 훈련셋에만 적용')

h2('5.3 2026-05-25 예측 결과')
bul(f'사용 모델: {best_nn} (RMSE·DA 복합 기준 최우수 NN)')
for d in DISTRICTS:
    nn_v = fc_nn[d]; ar_v = fc_ar.get(d, 0)
    bul(f'{d}: {best_nn} {nn_v:.2f}  /  ARIMA {ar_v:.2f}', sub=True)
bul(f'두 모델 6구 평균: {best_nn} {n_avg:.2f} / ARIMA {a_avg:.2f} — 방향 일치로 신뢰성 상호 보강')
