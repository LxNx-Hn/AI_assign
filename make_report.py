...
bul('과제 명세 기준: 2021.05.10 ~ 2026.05.04 (251주, 매주 월요일)')
...
bul('특성 공학에 week/month 삼각함수(4개) 포함 — 계절성 없음을 확인 후에도 탐색적으로 포함하되 모델이 학습 중 낮은 가중치 부여 시 실질 무효화')
...
bul('모델 선택 기준: RMSE 상위 10% 이내 후보 중 DA(방향정확도) 최대 모델 채택 — 단순 RMSE 최소화보다 방향 신뢰성 우선')
...
bul(f'평가 기간 차이: ARIMA ~52주 / NN ~40주 — 동일 기간 비교가 아님, RMSE 수치 해석 시 감안 필요')
...
bul(f'R² {best_r2:.3f}: 실제 가격 변동의 {best_r2*100:.0f}%를 모델이 설명')
...
h2('5.2 모델 성능 결론')
bul(f'ARIMA(walk-forward): RMSE {arima_avg_rmse:.3f}, DA {arima_avg_da:.1f}% — 이 테스트 기간 수치 1위')
bul(f'최우수 NN {best_nn}: RMSE {best_rmse:.3f}, DA {best_da:.1f}%, R² {best_r2:.3f}')
bul('ARIMA 우위 이유: 완만한 회복기에서 직전값 닻(anchor) 전략이 random-walk 데이터에 최적')
bul('NN 강점: 6개 구 지역 간 관계 동시 학습, 비선형 패턴 포착, 레짐 변화 국면에서 강건성')
bul('방법론 기여: 차분 타겟 전략으로 훈련·테스트 분포 불일치 문제 해소')
bul('공정 비교: ARIMA도 NN과 동일한 walk-forward 방식으로 평가 — one-shot 장기예측 vs 롤링 단기예측 비교 오류 방지')
...
bul('스케일러 범위 초과: 테스트 입력 일부가 [0,1] 범위 초과 — 훈련 데이터에만 fit하는 올바른 방법론의 불가피한 한계')
